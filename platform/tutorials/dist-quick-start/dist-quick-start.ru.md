# Предсобранный вариант поставки (dist) библиотеки bem-components. Подключаем блоки на страницу
...
Один из [вариантов поставки](https://ru.bem.info/platform/libs/bem-components/6.0.0/#Использование) (dist) библиотеки [bem-components](https://ru.bem.info/platform/libs/bem-components/6.0.0/) — это предварительно собранные файлы ([бандлы](https://ru.bem.info/methodology/build/#Введение)), которые подключаются ссылками в HTML страницы и не требуют совместимости с проектом. Это самый быстрый способ попробовать [блоки](https://ru.bem.info/methodology/key-concepts/#Блок) библиотеки в действии.
...
Мы подключим блоки `button` и `input` из библиотеки в форму приветствия, как показано на рисунке ниже. Имя пользователя при нажатии на кнопку отобразится в приветствии.
...
> [HTML-код](https://gist.github.com/innabelaya/a29d803145cbdb161ac5238a782c24f9) страницы с формой приветствия.
...
> При использовании готового HTML обновление шаблонов в библиотеке потребует вносить изменения в каждый блок вручную.
>
> При генерации HTML в браузере обновлять шаблоны вручную не потребуется. Однако генерируемая в браузере разметка может хуже индексироваться поисковыми системами.
